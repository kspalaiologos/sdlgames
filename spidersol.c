...
void drop_stack(int x, int y) {
    ...
    for(int i = 0; i < 10; i++) {
        ...
        if(j != -1) {
            if(x >= game.stacks[i].cards[j].x && x <= game.stacks[i].cards[j].x + CARD_WIDTH && y >= game.stacks[i].cards[j].y && y <= game.stacks[i].cards[j].y + CARD_HEIGHT) {
                stack = i;
                card = j;
                break;
            }
            ...
            if(x >= STACK_X_OFFSET + i * STACK_X_SPACING && x <= STACK_X_OFFSET + i * STACK_X_SPACING + CARD_WIDTH && y >= STACK_Y_OFFSET && y <= STACK_Y_OFFSET + CARD_HEIGHT) {
                stack = i;
                card = -1;
                break;
            }
    ...
    if((card != -1 && game.stacks[stack].cards[card].suit == game.stacks[dragging_stack].cards[dragging_stack_offset].suit && game.stacks[stack].cards[card].value == game.stacks[dragging_stack].cards[dragging_stack_offset].value - 1) || card == -1) {
        /* Move the stack. */
...
int main(void) {
    ...
    while (!done) {
        if(time(NULL) != secondsCurrent && (game.state == STATE_GAME_IDLE || game.state == STATE_GAME_DRAGGING_STACK)) {
            ...
            switch (event.type) {
                            ...
                            case STATE_GAME_IDLE:
                                for(int i = 0; i < 10; i++) {
                                    for(int j = game.stacks[i].num_cards - 1; j >= 0; j--) {
                                        if(j >= game.stacks[i].num_cards - game.stacks[i].visible_offset) {
                                            if(event.button.x >= game.stacks[i].cards[j].x && event.button.x <= game.stacks[i].cards[j].x + CARD_WIDTH &&
                                            event.button.y >= game.stacks[i].cards[j].y && event.button.y <= game.stacks[i].cards[j].y + CARD_HEIGHT) {
                                                ...
                                                for(k = j; k < game.stacks[i].num_cards; k++) {
                                                    if(game.stacks[i].cards[k].suit != color || game.stacks[i].cards[k].value != value) {
                                                        break;
                                                    }
                                                    value++;
                                                }
                ...
                case SDL_MOUSEMOTION:
                    if(game.state == STATE_GAME_DRAGGING_STACK) {
                        ...
                        highlightedMenu = -1;
                        for(int i = 0; i < 7; i++) {
                            if(event.motion.x >= menuButtons[i].x && event.motion.x >= menuButtons[i].y && event.motion.x <= menuButtons[i].x + menuButtons[i].w && event.motion.y <= menuButtons[i].y + menuButtons[i].h) {
                                highlightedMenu = i;
                                break;
                            }
                        }
                    ...
                    if(event.button.button == SDL_BUTTON_LEFT) {
                        switch(game.state) {
                            case STATE_GAME_IDLE:
                                ...
                                if(game.remainingExtraDeals > 0 && event.button.x >= EXTRA_CARDS_X_OFFSET && event.button.x <= EXTRA_CARDS_X_OFFSET + CARD_WIDTH * game.remainingExtraDeals
                                && event.button.y >= EXTRA_CARDS_Y_OFFSET && event.button.y <= EXTRA_CARDS_Y_OFFSET + CARD_HEIGHT) {
