...
void make_ai_move() {
  ...
  for (int i = 0; i < 8; i++)
    for (int j = 0; j < 8; j++)
      arr_board[i * 8 + j] = board[i][j] == B_EMPTY ? -1
                             : board[i][j] == B_RED ? 1
                                                    : 0;
...
int main() {
  ...
  SDL_Window *window = SDL_CreateWindow(
      "SDLReversi", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 314, 360,
  ...
  while (!done) {
            ...
            for (int j = 0; j < 8; j++) {
              if (board[i][j] == B_EMPTY) {
                if (mx >= r.x && mx < r.x + r.w && my >= r.y &&
                    my < r.y + r.h && is_move_possible(i, j)) {
                  // OK, make move.
    ...
    if (!game_over() && !animating && player == B_BLUE)
      make_ai_move();
        ...
        if (ImGui::MenuItem("Pass", "Ctrl+P")) {
          ...
          for (int i = 0; i < 8 && can_pass; i++)
            for (int j = 0; j < 8 && can_pass; j++)
              if (board[i][j] == B_EMPTY && is_move_possible(i, j))
                can_pass = false;
          if (can_pass) {
            player = B_BLUE;
          }
