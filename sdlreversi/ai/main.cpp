...
inline void print_result(int policy, int value) {
  cout << policy / HW << " " << policy % HW << " " << calc_result_value(value)
       << endl;
}

inline void print_result(Search_result result) {
  cout << idx_to_coord(result.policy) << " " << calc_result_value(result.value)
       << endl;
}
...
extern "C" int ai_js(int *arr_board, int level, int ai_player) {
  ...
  cout << "searched policy " << idx_to_coord(result.policy) << " value "
       << result.value << " nps " << result.nps << endl;
