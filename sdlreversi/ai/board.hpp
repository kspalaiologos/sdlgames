...
public:
  ...
  inline uint32_t hash() {
    ...
    return hash_rand_player[0][p[0]] ^ hash_rand_player[1][p[1]] ^
           hash_rand_player[2][p[2]] ^ hash_rand_player[3][p[3]] ^
           hash_rand_opponent[0][o[0]] ^ hash_rand_opponent[1][o[1]] ^
           hash_rand_opponent[2][o[2]] ^ hash_rand_opponent[3][o[3]];
  }
  ...
  inline uint64_t get_legal() { return calc_legal(player, opponent); }
  ...
  inline void pass() { swap(player, opponent); }
  ...
  inline int count_player() const { return pop_count_ull(player); }

  inline int count_opponent() const { return pop_count_ull(opponent); }

  inline int n_discs() const { return pop_count_ull(player | opponent); }
  ...
  inline bool is_end() {
    return (calc_legal(player, opponent) == 0ULL) &&
           (calc_legal(opponent, player) == 0ULL);
  }
...
struct Board_hash {
  size_t operator()(Board board) const { return board.hash(); }
};
  ...
  for (i = 0; i < 4; ++i) {
    for (j = 0; j < 65536; ++j) {
      ...
      while (pop_count_uint(hash_rand_player[i][j]) < 4)
        hash_rand_player[i][j] =
            myrand_uint_rev(); //(uint32_t)(rotate_180(myrand_ull()) >> 32);
      hash_rand_opponent[i][j] = 0;
      while (pop_count_uint(hash_rand_opponent[i][j]) < 4)
        hash_rand_opponent[i][j] =
            myrand_uint_rev(); //(uint32_t)(rotate_180(myrand_ull()) >> 32);
    }
  }
}
