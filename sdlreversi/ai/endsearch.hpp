...
inline int last1(Search *search, int alpha, int beta, uint_fast8_t p0) {
  ...
  if (n_flip == 0) {
    ...
    if (score <= 0) {
      ...
      if (score >= alpha) {
        n_flip =
            count_last_flip(search->board.opponent, search->board.player, p0);
        score -= 2 * n_flip;
      }
    } else {
      if (score >= alpha) {
...
inline int last2(Search *search, int alpha, int beta, uint_fast8_t p0,
                 uint_fast8_t p1, bool skipped) {
  ++search->n_nodes;
#if USE_END_PO & false
...
inline int last3(Search *search, int alpha, int beta, uint_fast8_t p0,
                 uint_fast8_t p1, uint_fast8_t p2, bool skipped) {
  ++search->n_nodes;
#if USE_END_PO
  if (!skipped) {
  ...
  if ((bit_radiation[p0] & bit_radiation[p2]) ||
      (bit_radiation[p1] & bit_radiation[p2])) {
...
inline int last4(Search *search, int alpha, int beta, uint_fast8_t p0,
                 uint_fast8_t p1, uint_fast8_t p2, uint_fast8_t p3,
                 bool skipped) {
  ++search->n_nodes;
#if USE_END_SC
...
int nega_alpha_end_fast(Search *search, int alpha, int beta, bool skipped,
                        bool stab_cut, const bool *searching) {
  ...
  if (0 < search->parity && search->parity < 15) {
    ...
    if (legal_copy) {
      if (search->n_discs == 59) {
        ...
        for (cell = first_bit(&legal_copy); legal_copy;
             cell = next_bit(&legal_copy)) {
  ...
  if (search->n_discs == 59) {
    uint64_t empties;
    uint_fast8_t p0, p1, p2, p3;
    for (uint_fast8_t cell = first_bit(&legal); legal;
         cell = next_bit(&legal)) {
      calc_flip(&flip, &search->board, cell);
      search->move(&flip);
      empties = ~(search->board.player | search->board.opponent);
...
int nega_alpha_end(Search *search, int alpha, int beta, bool skipped,
                   uint64_t legal, const bool *searching) {
  if (!global_searching || !(*searching))
    return SCORE_UNDEFINED;
  ...
  if (legal == 0ULL) {
    if (skipped)
      return end_evaluate(&search->board);
    search->board.pass();
    v = -nega_alpha_end(search, -beta, -alpha, true, LEGAL_UNDEFINED,
                        searching);
    search->board.pass();
    return v;
  }
  ...
  if (best_move != TRANSPOSE_TABLE_UNDEFINED) {
    if (1 & (legal >> best_move)) {
      ...
      g = -nega_alpha_end(search, -beta, -alpha, false, LEGAL_UNDEFINED,
                          searching);
  ...
  if (alpha < beta && legal) {
    ...
    // const int move_ordering_threshold = MOVE_ORDERING_THRESHOLD -
    // (int)(best_move != TRANSPOSE_TABLE_UNDEFINED);
    for (int move_idx = 0; move_idx < canput; ++move_idx) {
      // if (move_idx < move_ordering_threshold)
      swap_next_best_move(move_list, move_idx, canput);
      search->move(&move_list[move_idx].flip);
      g = -nega_alpha_end(search, -beta, -alpha, false,
                          move_list[move_idx].n_legal, searching);
      ...
      if (*searching) {
        ...
        if (v < g) {
          ...
          if (beta <= alpha) {
            register_tt(search, HW2 - search->n_discs, hash_code, first_alpha,
                        v, best_move, l, u, alpha, beta, searching);
            return alpha;
          }
        }
      } else
  ...
  register_tt(search, HW2 - search->n_discs, hash_code, first_alpha, v,
              best_move, l, u, alpha, beta, searching);
  return v;
}