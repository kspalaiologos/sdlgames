...
public:
  ...
  inline bool import_file_bin(string file) {
    ...
    for (i = 0; i < n_boards; ++i) {
      ...
      if (fread(&p, 8, 1, fp) < 1) {
        cerr << "book NOT FULLY imported " << n_book << " boards code 1"
             << endl;
        fclose(fp);
        return false;
      }
      if (fread(&o, 8, 1, fp) < 1) {
        cerr << "book NOT FULLY imported " << n_book << " boards code 2"
             << endl;
        fclose(fp);
        return false;
      }
      if (fread(&elem, 1, 1, fp) < 1) {
        cerr << "book NOT FULLY imported " << n_book << " boards code 3"
             << endl;
        fclose(fp);
        return false;
      }
      ...
      if (value < -HW2 || HW2 < value) {
        cerr << "book NOT FULLY imported " << n_book
             << " boards code 4 got value " << value << endl;
        fclose(fp);
        return false;
      }
      b.player = p;
  ...
  inline Book_value get_random(Board *b, int accept_value) {
    ...
    for (uint_fast8_t cell = first_bit(&legal); legal;
      ...
      if (value != -INF) {
        ...
        max_value = MY_MAX(max_value, value);
      }
    }
    Book_value res;
    if (policies.size() == 0) {
  ...
  inline int get_n_book() { return n_book; }
  ...
  inline void save_bin(string file, string bak_file) {
    ...
    for (auto itr = book.begin(); itr != book.end(); ++itr) {
      ...
      elem = MY_MAX(0, min(HW2 * 2, itr->second + HW2));
...
bool book_init(string file) { return book.init(file); }