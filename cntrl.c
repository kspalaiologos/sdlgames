...
void cntrlRender() {
    ...
    if(selectedSrcColumn != -1) {
        if(game.stacks[selectedSrcColumn].num_cards != 0) {
            SDL_Rect r;
            r.x = game.stacks[selectedSrcColumn].cards[game.stacks[selectedSrcColumn].num_cards - selectedSrcColumnIndex].x - 5;
            r.y = game.stacks[selectedSrcColumn].cards[game.stacks[selectedSrcColumn].num_cards - selectedSrcColumnIndex].y - 5;
            r.w = CARD_WIDTH + 10;
...
void try_move() {
    if(selectedColumn != selectedSrcColumn && ((game.stacks[selectedColumn].num_cards == 0) || (game.stacks[selectedColumn].cards[game.stacks[selectedColumn].num_cards - 1].suit == game.stacks[selectedSrcColumn].cards[game.stacks[selectedSrcColumn].num_cards - selectedSrcColumnIndex].suit && game.stacks[selectedColumn].cards[game.stacks[selectedColumn].num_cards - 1].value == game.stacks[selectedSrcColumn].cards[game.stacks[selectedSrcColumn].num_cards - selectedSrcColumnIndex].value - 1))) {
        // OK, we can move.
        ...
        for(int i = selectedSrcColumnIndex; i < game.stacks[selectedSrcColumn].num_cards; i++) {
            game.stacks[selectedColumn].cards[game.stacks[selectedColumn].num_cards] = game.stacks[selectedSrcColumn].cards[i];
        ...
        if(game.stacks[selectedSrcColumn].visible_offset <= 0)
            game.stacks[selectedSrcColumn].visible_offset = 1;
                    ...
                    if(selectedColumn == -1) {
                        ...
                        if(selectedSrcColumn == -1) {
                            ...
                            if(selectedColumn == selectedSrcColumn)
                                { selectedColumn++; if(selectedColumn == 10) selectedColumn = 0; }
                        }
                    }
                }
                break;
