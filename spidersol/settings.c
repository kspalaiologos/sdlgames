...
void settingsHandleMouseUp(int x, int y) {
    ...
    if (x <= SETTINGS_WND_OFFSET_X || x >= SETTINGS_WND_OFFSET_X + SETTINGS_WND_WIDTH || y <= SETTINGS_WND_OFFSET_Y ||
        y >= SETTINGS_WND_OFFSET_Y + SETTINGS_WND_HEIGHT) {
    ...
    switch (game.difficulty) {
        case 1:
            diffwidth = CARD_WIDTH + SETTINGS_CARDS_X_SPACING;
            break;
        case 2:
            diffwidth = CARD_WIDTH * 2 + SETTINGS_CARDS_X_SPACING * 2;
            break;
        case 3:
            diffwidth = CARD_WIDTH * 4 + SETTINGS_CARDS_X_SPACING * 4;
            break;
    }
    int baseX = SETTINGS_WND_OFFSET_X + CARD_WIDTH + 80, baseY = SETTINGS_WND_OFFSET_Y + 10;
    if (x > baseX && x < baseX + diffwidth && y > baseY && y < baseY + CARD_HEIGHT) {
