...
void compute_offsets(int * offsets) {
    ...
    for(int i = 0; i < 10; i++) {
        if(game.stacks[i].num_cards == 0)
            { offsets[i] = -1; continue; }
        if(game.stacks[i].num_cards == 1)
            { offsets[i] = 0; continue; }
        
        while(game.stacks[i].num_cards - 2 - offsets[i] >= 0 && game.stacks[i].cards[game.stacks[i].num_cards - 1 - offsets[i]].value - 1 == game.stacks[i].cards[game.stacks[i].num_cards - 2 - offsets[i]].value
        && game.stacks[i].cards[game.stacks[i].num_cards - 1 - offsets[i]].suit == game.stacks[i].cards[game.stacks[i].num_cards - 2 - offsets[i]].suit) {
            offsets[i]++;
        }
...
int has_moves() {
    ...
    // E.g., if we move a 2 to a 3, but the 2 was previously on a 3, this makes no sense unless an entire stack is getting folded.

    // In other words: Move only full stacks as defined from offsets[i] to stacks[i].num_cards, UNLESS the last card is an Ace
    // and this operation will result in a full stack from king, queen, jack, etc... until an ace.
    ...
    for(int i = 0; i < 10; i++) {
        for(int j = 0; j < 10; j++) {
            ...
            // Can be moved!
            if(game.stacks[i].cards[offsets[i]].value + 1 == game.stacks[j].cards[game.stacks[j].num_cards - 1].value
    ...
    for(int i = 0; i < 10; i++) {
        if(game.stacks[i].num_cards == 0 || offsets[i] == -1)
            continue;
